...
class ResNet(nn.Module):
    def __init__(self, inplanes=1, layers=(2, 2, 2, 2)):
        ...
        self.att1 = ScaledDotProductAttention(196, 14, 14, 14)
        self.att2 = ScaledDotProductAttention(40, 10, 10, 4)
        self.att3 = ScaledDotProductAttention(8, 4, 4, 2)
        self.att4 = ScaledDotProductAttention(2, 2, 2, 1)

        self.datt3 = ScaledDotProductAttention(1, 1, 1, 1)
        self.datt2 = ScaledDotProductAttention(2, 2, 2, 1)
        self.datt1 = ScaledDotProductAttention(8, 4, 4, 2)
    ...
    def forward(self, x):
        x_att = torch.mul(x, self.att1(x, x, x))
        x = torch.add(x, x_att)
        down1 = self.encoder1(x)

        down1_att = torch.mul(down1, self.att2(down1, down1, down1))
        down1 = torch.add(down1, down1_att)
        down2 = self.encoder2(down1)

        down2_att = torch.mul(down2, self.att3(down2, down2, down2))
        down2 = torch.add(down2, down2_att)
        down3 = self.encoder3(down2)

        down3_att = torch.mul(down3, self.att4(down3, down3, down3))
        down3 = torch.add(down3, down3_att)
        down4 = self.encoder4(down3)

        down4_att = torch.mul(down4, self.datt3(down4, down4, down4))
        down4 = torch.add(down4, down4_att)
        up3 = self.decoder3(down4, down3)

        up3_att = torch.mul(up3, self.datt2(up3, up3, up3))
        up3 = torch.add(up3, up3_att)
        up2 = self.decoder2(up3, down2)

        up2_att = torch.mul(up2, self.datt1(up2, up2, up2))
        up2 = torch.add(up2, up2_att)
...
class Net(torch.nn.Module):
    def __init__(self,
        ...
        self.fnet = MLP([dim] + [64, 32, 32] + [n_outputs])
    ...
    def observe(self, x, t, y, loss_type='MSE', x_te=None, x_tr=None, opt=None):
        if opt is not None:
            self.opt = opt
